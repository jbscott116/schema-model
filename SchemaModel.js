...
class SchemaModel {
	...
	constructor(schema, defaults) {
		if (!schema) throw new Error('The schema argument must be provided.');
		if (!isObject(schema)) throw new TypeError('The schema argument must be an Object.');
		...
		schemaKeys.forEach(function (key, i, arr) {
			...
			if (schemaModelValue === Object) {
				if (defaultValue === undefined) {
					...
					if (defaultValue.constructor.name !== 'Object') throw new TypeError(`Default value for key ${key} is not an Object`);
			...
			if (schemaModelValue === String) {
				if (defaultValue === undefined) {
					...
					if (defaultValue.constructor.name !== 'String') throw new TypeError(`Default value for key ${key} is not a String`);
			...
			if (schemaModelValue === Number) {
				if (defaultValue === undefined) {
					...
					if (defaultValue.constructor.name !== 'Number') throw new TypeError(`Default value for key ${key} is not a Number`);
					self.model[key] = self.blank[key] = defaultValue;
				}
			}
			...
			if (schemaModelValue === Boolean) {
				if (defaultValue === undefined) {
					...
					if (defaultValue.constructor.name !== 'Boolean') throw new TypeError(`Default value for key ${key} is not a Boolean`);
					self.model[key] = self.blank[key] = defaultValue;
				}
			}
			...
			if (schemaModelValue === Function) {
				if (defaultValue === undefined) {
					...
					if (defaultValue.constructor.name !== 'Function') throw new TypeError(`Default value for key ${key} is not a Function`);
					self.model[key] = self.blank[key] = defaultValue;
				}
			}
			...
			if (schemaModelValue === Array) {
				if (defaultValue === undefined) {
					...
					if (defaultValue.constructor.name !== 'Array') throw new TypeError(`Default value for key ${key} is not an Array`);
					self.model[key] = self.blank[key] = defaultValue;
				}
			}
			...
			if (schemaModelValue && schemaModelValue.name && !defaultTypes.includes(schemaModelValue.name)) {
				if (defaultValue === undefined) {
					self.model[key] = self.blank[key] = null;
				} else {
	...
	apply(props, appendArrayItems) {
		if (!props) throw new Error('An object is required for the \'props\' argument!');
		if (!props instanceof Object) throw new Error('Props argument must be an Object!');
		if (Object.keys(props).length == 0) return this.model;
		...
		schemaKeys.forEach(function (key, i, arr) {
			...
			if (!isUndefined(propValue)) {
				...
				if (isObject(schemaModelValue) && !(schemaModelValue instanceof Array)) {
					...
					if (isString(propValue)) {
						...
						if (Number.isNaN(parseInt(propValue)) || Number.isNaN(parseFloat(propValue)))
							...
							if (isFloatString(propValue)) self.model[key] = parseFloat(propValue);
							else self.model[key] = parseInt(propValue);
						}
					}
						...
						throw new TypeError(
							`Object property ${key} of value, ${propValue}, must be either a number or a string that may be parsed as a number.`
						);
					}
					...
					if (!(propValue instanceof Array)) {
						...
						if (!appendArrayItems) self.model[key] = [];
				...
				} else if (schemaModelValue && schemaModelValue.name && !defaultTypes.includes(schemaModelValue.name)) {
