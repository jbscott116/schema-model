...
describe('SchemaModel', () => {
	...
	describe('Default values', () => {
		...
		test('setting defaults in constructor throws no errors', () => {
			expect(() => {
				return new SchemaModel(TYPE_SCHEMA, DEFAULTS);
			}).not.toThrow();
		});
		...
		test('throws errors when default type does not match schema type', () => {
			expect(() => {
				return new SchemaModel(TYPE_SCHEMA, INVALID_DEFAULTS);
			}).toThrow();
		});
	});
	...
	describe('apply(properties)', () => {
		...
		describe('Valid value types when applying', () => {
			...
			test('valid number (parse string)', () => {
				expect(() => {
					return typeModel.apply({ number: '5' });
				}).not.toThrow();
			});
			...
			test('valid number (float)', () => {
				expect(() => {
					return typeModel.apply({ number: 5.5 });
				}).not.toThrow();
			});
			test('valid string', () => {
				expect(() => {
					return typeModel.apply({ string: 'x' });
				}).not.toThrow();
			});
			test('valid object', () => {
				expect(() => {
					return typeModel.apply({ obejct: {} });
				}).not.toThrow();
			});
			test('valid array', () => {
				expect(() => {
					return typeModel.apply({ array: [] });
				}).not.toThrow();
			});
			test('valid function', () => {
				expect(() => {
					return typeModel.apply({ function: function () { } });
				}).not.toThrow();
			});
			test('valid boolean (true)', () => {
				expect(() => {
					return typeModel.apply({ boolean: true });
				}).not.toThrow();
			});
			test('valid boolean (false)', () => {
				expect(() => {
					return typeModel.apply({ boolean: false });
				}).not.toThrow();
			});
			test('valid boolean (string)', () => {
				expect(() => {
					return typeModel.apply({ boolean: 'valid' });
				}).not.toThrow();
			});
			test('valid boolean (1)', () => {
				expect(() => {
					return typeModel.apply({ boolean: 1 });
				}).not.toThrow();
			});
			test('valid boolean (0)', () => {
				expect(() => {
					return typeModel.apply({ boolean: 0 });
				}).not.toThrow();
			});
			test('valid boolean (array)', () => {
				expect(() => {
					return typeModel.apply({ boolean: [] });
				}).not.toThrow();
			});
		});
		...
		describe('TypeErrors when applying different value types', () => {
			...
			test('invalid number (cannot parse)', () => {
				expect(() => {
					return typeModel.apply({ number: 'x' });
				}).toThrow();
			});
			test('invalid number (invalid type)', () => {
				expect(() => {
					return typeModel.apply({ number: [] });
				}).toThrow();
			});
			test('invalid string (object)', () => {
				expect(() => {
					return typeModel.apply({ string: {} });
				}).toThrow();
			});
			...
			test('invalid array (string)', () => {
				expect(() => {
					return typeModel.apply({ array: 'hello, world' });
				}).toThrow();
			});
			test('invalid function (string)', () => {
				expect(() => {
					return typeModel.apply({ function: 'invalid' });
				}).toThrow();
			});
		});
		...
		describe('String -> Number(float) conversions', () => {
			...
			test('throws no errors', () => {
				expect(() => {
					return floatModel.apply(properties);
				}).not.toThrow();
			});
	...
	describe('Schemas for array items', () => {
		describe('constructor()', () => {
			test('SchemaModel constructs successfully', () => {
				expect(() => {
					return new SchemaModel(ARRAY_MODEL_SCHEMA);
				}).not.toThrow();
			});
		...
		describe('apply()', () => {
			...
			var properties = {
				name: 'dog',
				list: [{ dog: true, cat: false, car: true }],
			};
			test('Applying arrayModel to array items throws no errors', () => {
				expect(() => {
					return myArrayModel.apply(properties);
				}).not.toThrow();
			});
	...
	describe('Deep schemas and array schemas', () => {
		describe('constructor()', () => {
			test('construting deep schema model throws no errors', () => {
				expect(() => {
					return new SchemaModel(DEEP_SCHEMA);
				}).not.toThrow();
			});
		});
		...
		var additionalPostsAndComments = {
			posts: [
				{
					title: 'another post',
					comments: [{ id: '561651', content: 'Yet another comment' }],
				},
			],
		};
		...
		describe('apply()', () => {
			test('applying to deep model throws no errors', () => {
				expect(() => {
					return deepModel.apply(properties);
				}).not.toThrow();
			});
	...
	describe('Non-standard constructor types', () => {
		const NON_STD_SCHEMA = {
			aCar: Car,
		};

		var nonStdSchemaModel = new SchemaModel(NON_STD_SCHEMA);

		test('constructor throws no errors', () => {
			expect(() => {
				return new SchemaModel(NON_STD_SCHEMA);
			}).not.toThrow();
		});

		test('non-standard properties are correctly applied to the model.', () => {
			var testingApply = new SchemaModel(NON_STD_SCHEMA);
			var myNewCar = new Car();
			expect(() => {
				testingApply.apply({ aCar: myNewCar });
			}).not.toThrow();
